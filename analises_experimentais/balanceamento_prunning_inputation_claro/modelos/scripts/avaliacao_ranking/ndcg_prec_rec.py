...
def pegatrue(y_true, y_score, k=10):
    """Discounted cumulative gain (DCG) at rank k
    Parameters
    ----------
    y_true : array-like, shape = [n_samples]
...
#   for i in range(1, 101,1): 
      n=ndcg_score(y_traina, y_score, k=i)
#      n=ndcg_at_k(y_traina, y_score)

      p=ranking_precision_score(y_traina, y_score, k=i)
      t=pegatrue(y_traina,y_score,k=i)
#      print str(i)+" "+str(n)
      if i<num_rel:
         vecn.append(n)
         veci2.append(i)
...
#   plt.step(veci, vecn, color='b', alpha=0.2, where='post')
   ...
   plt.plot(veci, np.array(vect)/descobrerel(y_traina), 'g-',label="Recall@N")
...
#
   ...
   strtitulo="Precision@N and NDCG@N for "+train
#   plt.title(strtitulo)
#   plt.show()
#   #plt.savefig('foo.png')
   plt.savefig('foo.pdf')
