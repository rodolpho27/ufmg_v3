...
from modelos.newknn import KNeighborsClassifier
...
#		individualfeatures=(x_recargalagplano,x_features_iuri_lag,x_recarga_lag_vivo,x_recharge_interval,x_recarga,x_mailing,x_servico_clientes,x_dia_mes,x_dia_semana,x_clientes,x_qtdcalls,x_fidelidade,x_hora,x_prezao,x_whitelist)				
	...
	def run_timestamp(self, database, npz_type):
		...
		try:
			loaded=np.load('{}_get_timestamp.npz'.format(npz_type))
			try:
				x = loaded['x_comp']
			except:
				x = loaded['x']
			...
			np.savez_compressed('{}_get_timestamp.npz'.format(npz_type), x=x)
		return x
        ...
        def run_knn(self, k2,outputfile):
                ...
                print('Running model...')
                forest = KNeighborsClassifier(train=self.x_train, test=self.x_test , k=k2, scores=outputfile+"_"+str(k2)+scores, Y_train=self.y_train)
...
#               forest.model_evaluation(prediction, self.y_test)
	...
	def janela_temporal(self,timestamp,k,y_trainaux,x_trainaux,namefeature):
#		self.timestamp = self.run_timestamp(npz_type='treino', database='files3_claro_mig')

#		y_trainaux, x_trainaux, individual_features= self.run_features(npz_type='treino', database='files3_claro_mig')
		...
		for i in range(5):
			...
			f1,prec,rec,acc = self.run_knn(k,"scores_part"+str(i)+"_"+namefeature)
			f1avg.append(f1)
			precavg.append(prec)
			recavg.append(rec)
			accavg.append(acc)
...
#	        print ("Recall: %f(%f)" % (np.mean(recavg),np.std(recavg)))
	...
	def run(self):
		timestamp = self.run_timestamp(npz_type='treino', database='files3_claro_mig')

		y_trainaux,  individual_features= self.run_features(npz_type='treino', database='files3_claro_mig')

		resultados={}
		for k in [5, 10, 30]:
			print ("AVG resultados para k="+str(k))
			for namefeature,x_value in individual_features.items():
				print ("janela temporal para features "+namefeature)			
				resultados[namefeature]=self.janela_temporal(timestamp,k,y_trainaux,x_value,namefeature)

#		print ("janela temporal para features tudo")			
#		resultados["tudo"]=self.janela_temporal(timestamp,y_trainaux,individual_features["tudo"],"tudo")
			


			for namefeature,results in resultados.items():
				print ("----------------------------")					
				print ("AVG Average of 5 parts for "+namefeature+":")
				print ("AVGf1_binary: %f(%f)" % (np.mean(results[0]),np.std(results[0])))
			        print ("AVGAccuracy: %f(%f)" % (np.mean(results[3]), np.std(results[3])))
			        print ("AVGPrecision: %f(%f)" % (np.mean(results[1]),np.std(results[1])))
			        print ("AVGRecall: %f(%f)" % (np.mean(results[2]),np.std(results[2])))
