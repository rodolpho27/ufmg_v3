...
class AnalysisFeaturesIuri:  
    ...
    def _get_recharges(self):
        ...
        for client, type_, value, date in res:
            if client not in to_append:
                to_append[client] = {'fifteen': 0, 'twenty': 0, 'thirteen': 0}
            if value:
                if int(value) == 15:
    ...
    def run(self, outputfile):
        ...
        start = timeit.default_timer()
        index_set = ['label', 'planos_valor', 'plano_minutos', 'plano_dados', '$15', '$20', '$13', 'lag_m_recarga', 'lag_2_recarga', 'lag_2_pct', 'recarga retomada']
        ...
        for call in self.calls:
            ...
            if client in self.count_recharges:
                csvfile.write(',{},{},{},{},{},{},{}'.format(self.count_recharges[client]['fifteen'], self.count_recharges[client]['twenty'], 
                                                    self.count_recharges[client]['thirteen'],np.mean(np.asarray(self.interval_dates[client])),
    ...
    def gen_features(self):
        ...
        y = np.empty([len(self.calls) + len(self.sales)])
        x = np.empty([len(self.calls) + len(self.sales), 10])
        ...
        for call in self.calls:
            ...
            if client in self.clients_plans:
                ...
                if len(client_data) > 0:
                    client_data.append(-1)
                y[index] = 0
                x[index][0] = np.mean(np.asarray(client_values)) if client_values else -1
                x[index][1] = np.mean(np.asarray(client_minutes)) if client_minutes else -1
                x[index][2] = np.mean(np.asarray(client_data)) if client_data else -1
                ...
                x[index][2] = -1
            if client in self.count_recharges:
                x[index][3] = self.count_recharges[client]['fifteen'] if client in self.count_recharges else -1
                x[index][4] = self.count_recharges[client]['twenty'] if client in self.count_recharges else -1
                x[index][5] = self.count_recharges[client]['thirteen'] if client in self.count_recharges else -1
                x[index][6] = np.mean(np.asarray(self.interval_dates[client]))
                x[index][7] = self.interval_dates[client][0] if client in self.interval_dates and len(self.interval_dates[client]) >= 1 else -1
                x[index][8] = self.interval_dates[client][0]+self.interval_dates[client][1] if client in self.interval_dates and len(self.interval_dates[client]) >= 2 else -1
                x[index][9] = 1 if len([i for i in np.asarray(self.interval_dates[client])==60 if i == True]) > 0 else 0
            else:
                x[index][3] = -1
                x[index][4] = -1
                x[index][5] = -1
