...
class AnalysisFeaturesIuriLag:  
    ...
    def gen_features(self):
        ...
        x = np.empty([len(self.calls_clients) + len(self.sales_clients), 9])
        index = 0
        for call in self.calls_clients:
            client = self.calls_clients[call]
            x[index][0] = self.to_append_online[client]['rec_online_freq'] if client in self.to_append_online else -1
            x[index][1] = self.to_append_online[client]['rec_online_media'] if client in self.to_append_online else -1
            x[index][2] = self.to_append_online[client]['rec_online_std'] if client in self.to_append_online else -1
            x[index][3] = self.to_append_prezao[client]['lag_1'] if client in self.to_append_prezao else -1
            x[index][4] = self.to_append_prezao[client]['mean'] if client in self.to_append_prezao else -1
            x[index][5] = self.to_append_prezao[client]['std'] if client in self.to_append_prezao else -1 
            x[index][6] = self.to_append_sos[client]['lag_1'] if client in self.to_append_sos else -1
            x[index][7] = self.to_append_sos[client]['mean'] if client in self.to_append_sos else -1
            x[index][8] = self.to_append_sos[client]['std'] if client in self.to_append_sos else -1
            index += 1
        for call in self.sales_clients:
            client = self.sales_clients[call]
            x[index][0] = self.to_append_online[client]['rec_online_freq'] if client in self.to_append_online else -1
...
if __name__ == '__main__':
    ...
    planos = AnalysisFeaturesIuriLag()    
