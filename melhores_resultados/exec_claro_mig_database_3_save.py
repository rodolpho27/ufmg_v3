...
#		forest.model_evaluation(prediction, self.y_test)
	...
	def run(self):
		self.y_train, self.x_train = self.run_features(npz_type='treino', database='files3_claro_mig', useplans=False,usehistoric=False)
		self.y_test, self.x_test = self.run_features(npz_type='teste', database='files3_claro_mig_model', useplans=False,usehistoric=False)
