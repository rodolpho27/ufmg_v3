...
class ExecuteClaroMig:
	...
	def run_features(self, database, npz_type, useplans=True,usehistoric=True):
		...
		'''
                print('Labels...')
                try:
                        loaded=np.load('{}_labels_vm.npz'.format(npz_type))
                        y = loaded['y']
                except:         
                        print("Generating features...")
                        analysis = FeaturesLabel(database)
                        y = analysis.gen_features()
                        np.savez_compressed('{}_labels_vm.npz'.format(npz_type),  y=y)
...
#		forest.model_evaluation(prediction, self.y_test)
        ...
        def run(self):
                self.y_train, self.x_train = self.run_features(npz_type='treino', database='files3_vivo_mig', useplans=False,usehistoric=False)
                self.y_test, self.x_test = self.run_features(npz_type='teste', database='files3_vivo_mig_model', useplans=False,usehistoric=False)
